...
document.getElementById('toggleAdd').onclick = () => {
  if (addRow.classList.contains('hidden')) openForm(null);
  else closeForm();
};
...
async function loadTasks() {
  try {
    ...
    tasks.forEach(t => {
      if (!t.people) t.people = ['You'];
      t.date = t.due_date ? t.due_date.slice(0, 10) : '—';
      if (!t.notes) t.notes = '—';
      if (!t.status) t.status = 'upcoming';
      if (!t.priority) t.priority = 'Medium';
      t.done = t.status === 'published';
    });
    render();
  } catch {
    list.innerHTML = '<div style="padding:2rem;opacity:.5">Could not load tasks. Is the server running? (node server.js)</div>';
  }
}
...
async function saveTask() {
  ...
  const data = {
    ...
    due_date: document.getElementById('i-date').value || null,
  ...
  try {
    ...
    console.error(err);
    alert('Could not save task. Check the console.');
  }
}
...
async function toggleDone(id) {
  ...
  if (!task) return;
  const newStatus = task.done ? 'upcoming' : 'published';
  try {
...
function fmtDate(d) {
  if (!d || d === '—') return '—';
  const dt = new Date(d);
  return isNaN(dt) ? d : dt.toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' });
}
...
function render() {
  ...
  filtered.forEach(t => {
    ...
    row.querySelector('.check').onclick = () => toggleDone(t.id);
